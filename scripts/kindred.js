  ...
  const app = createApp({
    data() {
      return {
        ...
        deviceHasTouch: true,
        allPlayerRanks: modules.AllPlayerRanks,
        currency: new modules.Currency(),
        ...
        appTutorialSteps: modules.TutorialSteps,
        appTutorialBoardPieces: modules.ConstructLevel('010013110020010303112100220221213211102001200313120', true),
        appTutorialIsInPlay: false,
        appTutorialMePiece: new modules.PieceModel({ color: 'var(--color2)' }),
        appNotificationMessage: '',
        appSettingsAddBonusToScoreInterval: null,
        appSettingsModes: modules.Modes,
        ...
        appSettingsThemes: modules.Themes,
        ...
        gameCurrentLevel: new modules.SingleLevelModel({}),
        ...
        gameLastHighScore: new modules.ScoreModel({}),
        gameScoreToShare: new modules.ScoreModel({}),
        ...
        gameCurrentHintText: 'Select cats that share <b>at least 2</b> of my attributes: color, shape, or pattern.',
        gameCurrentHasAnyPieceEverBeenSelected: false,
        gameLikenessNudgeHasBeenShown: false,
        gameClickMeNudgeHasBeenShown: false,
        userSettingsUseCats: true,
        userDailyLevel: 0,
    ...
    methods: {
          ...
          if (_perfectMatch) {
            ...
            if (this.gameCurrentNumberOfMisses >= 1) {
              ...
              if (_totalPossibleLikePieces === 0) {
                ...
                this.gameCurrentHintText = _totalPossibleLikePieces === 1 ? 'There is only <b>' + _totalPossibleLikePieces + '</b> cat like me.' : 'There are a total of <b>' + _totalPossibleLikePieces + '</b> cats like me.';
              }
      ...
      CreateConfetti(_hue = null, _rank = null) {
        ...
        requestAnimationFrame(() => {
          ...
          for (let x = 0; x < count; x++) {
            ...
            confetto.bc = 'hsl(' + hue + ',80%, ' + lightness + '%)';
            confetto.r = +'0deg';
      ...
      async SetUserBasedOnRank(_rank, _confetti = false) {
        ...
        await modules.SaveData('userRank', this.userRank);
      },
      ...
      NewBoard() {
        ...
        if (_board === null && noMoreLevels) {
          ...
          _board = modules.ConstructLevel(modules.CreateLevelSource(), true);
        }
      ...
      TogglePieceSelection(_piece) {
        ...
        if (!this.gameLikenessNudgeHasBeenShown) {
          ...
          if (_likeness < 2) {
            this.appVisualStateShowElementHint = true;
            this.gameCurrentHintText = 'Select cats that share <b>at least 2</b> of my attributes: color, shape, or pattern.';
          }
          this.gameLikenessNudgeHasBeenShown = true;
        }
      ...
      ShareApp() {
        ...
        if (navigator.share) {
          ...
          _shareObject = 'https://bigtentgames.com/kindred.html';
...
${this.NumberWithCommas(this.gameScoreToShare.value)} pts - ${this.gameScoreToShare.numberOfClears} lvl${this.gameScoreToShare.numberOfClears === 1 ? '' : 's'} ${this.gameScoreToShare.numberOfPerfectClears > 0 ? '(' + this.gameScoreToShare.numberOfPerfectClears + ' perfect)' : ''}`;
      ...
      GiveFeedback() {
        window.location = 'mailto:bigtentgames@icloud.com?subject=Feedback - Kindred Cats v' + this.currentVersion;
      },
      ...
      RestartGame() {
        ...
        this.gameCurrentHintText = 'Select cats that share <b>at least two</b> of my attributes (color, shape, pattern).';
      ...
      async SelectTheme(_theme) {
        ...
        await modules.SaveData('userSettingsTheme', JSON.stringify(_theme.name));
      },
        ...
        if (!this.gameCurrentIsPaused && !this.appVisualStateShowPageGameOver) {
          if (this.gameCurrentNumberOfClears === 0 && this.userHighScoresEasy.length === 0) {
            ...
            if (!this.appSettingsInfiniteMode.isSelected && !this.appVisualStateShowElementHint && !this.gameClickMeNudgeHasBeenShown && this.appSettingsCurrentGameMode.starttime - this.gameCurrentTimer >= 25000 && this.gameCurrentTimer % 1000 === 0) {
              this.appVisualStateShowElementHint = true;
              this.gameCurrentHintText = 'Remember to click the me cat to check your work and move to the next level.';
              this.gameClickMeNudgeHasBeenShown = true;
      ...
      async EndGame() {
        note('EndGame() called');
        let _score = new modules.ScoreModel({
        ...
        await modules.SaveData('userRank', this.userRank);
        await modules.SaveData('userNumberOfPerfectBasicGames', this.userNumberOfPerfectBasicGames);
        await modules.SaveData('userHighScoresEasy', JSON.stringify(this.userHighScoresEasy));
        await modules.SaveData('userHighScoresInfinite', JSON.stringify(this.userHighScoresInfinite));
      },
      ...
      async SaveUserHighScoreEasy() {
        note('SaveUserHighScoreEasy() called');
        await modules.SaveData('userHighScoresEasy', JSON.stringify(app.userHighScoresEasy));
      },
      ...
      async SetUseHardCoreModeToTrue(event) {
        ...
        if (!this.userSettingsUseHardCoreMode) {
          ...
          await modules.SaveData('userSettingsUseHardCoreMode', this.userSettingsUseHardCoreMode);
        }
      },

      async SetUseHardCoreModeToFalse() {
      ...
      async ToggleUsingHints(event) {
        ...
        await modules.SaveData('userSettingsUseHints', this.userSettingsUseHints);
      },

      async ToggleUsingCats(event) {
        ...
        this.GetRandomWallpaper();
        await modules.SaveData('userSettingsUseCats', this.userSettingsUseCats);
      },

      async ToggleUsingAltPattern(event) {
        ...
        await modules.SaveData('userSettingsUseAltPatterns', this.userSettingsUseAltPatterns);
      },

      async ToggleUsingSound(event) {
        ...
        await modules.SaveData('userSettingsUseSoundFX', this.userSettingsUseSoundFX);
      },

      async TogglePlayingMusic(event) {
        ...
        await modules.SaveData('userSettingsPlayMusic', this.userSettingsPlayMusic);
      },

      async ToggleUsingDarkMode(event) {
        ...
        await modules.SaveData('userSettingsUseDarkMode', this.userSettingsUseDarkMode);
      },

      async ToggleGamePause(event, _value) {
        ...
        await modules.SaveData('gameCurrentIsPaused', this.gameCurrentIsPaused);
      },
      ...
      async GetUserSettings() {
        ...
        // return;
        let _modes = await modules.GetData('appSettingsModes');
        ...
        let _userRank = await modules.GetData('userRank');
        ...
        let _userSettingsUseHardCoreMode = await modules.GetData('userSettingsUseHardCoreMode');
        ...
        let _userNumberOfPerfectBasicGames = await modules.GetData('userNumberOfPerfectBasicGames');
        ...
        let _hints = await modules.GetData('userSettingsUseHints');
        if (_hints !== undefined && _hints !== null) {
          _hints = JSON.parse(_hints);
          this.userSettingsUseHints = _hints;
        }

        let _userSettingsUseCats = await modules.GetData('userSettingsUseCats');
        ...
        let _userSettingsUseAltPatterns = await modules.GetData('userSettingsUseAltPatterns');
        ...
        let _userHighScoresEasy = await modules.GetData('userHighScoresEasy');
        try {
        ...
        let _gameCurrentHasAnyPieceEverBeenSelected = await modules.GetData('gameCurrentHasAnyPieceEverBeenSelected');
        try {
        ...
        let _gameLikenessNudgeHasBeenShown = await modules.GetData('gameLikenessNudgeHasBeenShown');
        try {
        ...
        let _gameClickMeNudgeHasBeenShown = await modules.GetData('gameClickMeNudgeHasBeenShown');
        try {
      ...
      async ClearAllUserScores() {
        note('ClearAllUserScores() called');
        let _confirm = window.confirm('Are you sure you want to clear all of your scores?');
        if (_confirm) {
          this.appVisualStateShowNotification = false;
          this.gameLastHighScore = new modules.ScoreModel({});
          this.gameScoreToShare = new modules.ScoreModel({});
      ...
      async RestoreCurrentGame() {
        ...
        let _appSettingsModeIntervalIncrement = await modules.GetData('appSettingsModeIntervalIncrement');
        try {
        ...
        let _appSettingsPieceSize = await modules.GetData('appSettingsPieceSize');
        try {
        ...
        let _appSettingsTotalNumberOfBoardPieces = await modules.GetData('appSettingsTotalNumberOfBoardPieces');
        try {
        ...
        let _appSettingsBoardGridSize = await modules.GetData('appSettingsBoardGridSize');
        try {
        ...
        let _appVisualStateShowPageHome = await modules.GetData('appVisualStateShowPageHome');
        try {
        ...
        let _appVisualStateShowPageHowToPlay = await modules.GetData('appVisualStateShowPageHowToPlay');
        try {
        ...
        let _appVisualStateShowPageSettings = await modules.GetData('appVisualStateShowPageSettings');
        try {
        ...
        let _appVisualStateShowElementHint = await modules.GetData('appVisualStateShowElementHint');
        try {
        ...
        let _appVisualStateShowElementFlyaway = await modules.GetData('appVisualStateShowElementFlyaway');
        try {
        ...
        let _gameCurrentIsPaused = await modules.GetData('gameCurrentIsPaused');
        try {
        ...
        let _gameDailyChallenge = await modules.GetData('gameDailyChallenge');
        try {
          if (_gameDailyChallenge !== undefined && _gameDailyChallenge !== null) {
            _gameDailyChallenge = JSON.parse(_gameDailyChallenge);
            _letAllLevels = new modules.AllLevelsModel({ date: new Date(_gameDailyChallenge.date) });
            _gameDailyChallenge.allLevels.forEach((level) => {
              let _level = new modules.SingleLevelModel({ me: level.me, completed: level.completed });
              level.board.forEach((piece) => {
                _level.board.push(new modules.PieceModel(piece));
              });
        ...
        let _gameCurrentIsGameDailyChallenge = await modules.GetData('gameCurrentIsGameDailyChallenge');
        try {
        ...
        let _gameDailyChallengeHasBeenStarted = await modules.GetData('gameDailyChallengeHasBeenStarted');
        try {
        ...
        let _gameCurrentAllLevels = await modules.GetData('gameCurrentAllLevels');
        try {
        ...
        let _gameCurrentLevel = await modules.GetData('gameCurrentLevel');
        try {
          if (_gameCurrentLevel !== undefined && _gameCurrentLevel !== null) {
            _gameCurrentLevel = JSON.parse(_gameCurrentLevel);
            let _level = new modules.SingleLevelModel({ me: _gameCurrentLevel.me, completed: _gameCurrentLevel.completed });
            if (this.gameCurrentIsGameDailyChallenge) {
        ...
        let _gameCurrentMePiece = await modules.GetData('gameCurrentMePiece');
        try {
          if (_gameCurrentMePiece !== undefined && _gameCurrentMePiece !== null) {
            _gameCurrentMePiece = JSON.parse(_gameCurrentMePiece);
            this.gameCurrentMePiece = new modules.PieceModel(_gameCurrentMePiece);
          }
        } catch (_error) {
        ...
        let _gameCurrentBoardPieces = await modules.GetData('gameCurrentBoardPieces');
        try {
          if (_gameCurrentBoardPieces !== undefined && _gameCurrentBoardPieces !== null) {
            ...
            _gameCurrentBoardPieces.forEach((piece) => {
              this.gameCurrentBoardPieces.push(new modules.PieceModel(piece));
            });
          }
        } catch (_error) {
        ...
        let _gameCurrentStartingTime = await modules.GetData('gameCurrentStartingTime');
        try {
        ...
        let _gameCurrentTimer = await modules.GetData('gameCurrentTimer');
        try {
        ...
        let _gameCurrentNumberOfClears = await modules.GetData('gameCurrentNumberOfClears');
        try {
        ...
        let _gameCurrentNumberOfPerfectMatches = await modules.GetData('gameCurrentNumberOfPerfectMatches');
        try {
        ...
        let _gameCurrentIsUserGuessWrong = await modules.GetData('gameCurrentIsUserGuessWrong');
        try {
        ...
        let _gameCurrentNumberOfFails = await modules.GetData('gameCurrentNumberOfFails');
        try {
        ...
        let _gameCurrentNumberOfMisses = await modules.GetData('gameCurrentNumberOfMisses');
        try {
        ...
        let _gameCurrentHintText = await modules.GetData('gameCurrentHintText');
        try {
        ...
        let _gameCurrentTotalScore = await modules.GetData('gameCurrentTotalScore');
        try {
      ...
      async InitializeGame() {
        ...
        let _onemoretime = await modules.GetData('onemoretime');
        try {
          if (_onemoretime !== undefined && _onemoretime !== null) {
            _onemoretime = JSON.parse(_onemoretime);
            if (_onemoretime) {
              await modules.SaveData('onemoretime', false);
            }
          }
        } catch (_error) {
        ...
        let _storedVersion = await modules.GetData('storedVersion');
        try {
        ...
        let _userCurrency = await modules.GetData('userCurrency');
        try {
        ...
        let _userSettingsActionItems = await modules.GetData('userSettingsActionItems');
        try {
        ...
        let _gameCurrentIsGameOver = await modules.GetData('gameCurrentIsGameOver');
        try {
      ...
      async HandleOnPageHideEvent(_clearInterval = true) {
        if (!this.appIsBeingReset) {
          ...
          if (this.appSettingsSaveSettings) {
            await modules.SaveData('storedVersion', this.currentVersion);
            await modules.SaveData('appSettingsModes', JSON.stringify(this.appSettingsModes));
            await modules.SaveData('appSettingsModeIntervalIncrement', this.appSettingsModeIntervalIncrement);
            await modules.SaveData('appSettingsPieceSize', this.appSettingsPieceSize);
            await modules.SaveData('appSettingsTotalNumberOfBoardPieces', this.appSettingsTotalNumberOfBoardPieces);
            await modules.SaveData('appSettingsBoardGridSize', this.appSettingsBoardGridSize);
            await modules.SaveData('appVisualStateShowPageHome', JSON.stringify(this.appVisualStateShowPageHome));
            await modules.SaveData('appVisualStateShowPageHowToPlay', JSON.stringify(this.appVisualStateShowPageHowToPlay));
            await modules.SaveData('appVisualStateShowPageSettings', JSON.stringify(this.appVisualStateShowPageSettings));
            await modules.SaveData('appVisualStateShowElementHint', JSON.stringify(this.appVisualStateShowElementHint));
            await modules.SaveData('appVisualStateShowElementFlyaway', JSON.stringify(this.appVisualStateShowElementFlyaway));
            await modules.SaveData('gameDailyChallenge', JSON.stringify(this.gameDailyChallenge));
            await modules.SaveData('gameCurrentIsGameDailyChallenge', JSON.stringify(this.gameCurrentIsGameDailyChallenge));
            await modules.SaveData('gameDailyChallengeHasBeenStarted', JSON.stringify(this.gameDailyChallengeHasBeenStarted));
            await modules.SaveData('gameCurrentAllLevels', JSON.stringify(this.gameCurrentAllLevels));
            await modules.SaveData('gameCurrentLevel', JSON.stringify(this.gameCurrentLevel));
            await modules.SaveData('gameCurrentIsGameOver', JSON.stringify(this.gameCurrentIsGameOver));
            await modules.SaveData('gameCurrentMePiece', JSON.stringify(this.gameCurrentMePiece));
            await modules.SaveData('gameCurrentBoardPieces', JSON.stringify(this.gameCurrentBoardPieces));
            await modules.SaveData('gameCurrentStartingTime', this.gameCurrentStartingTime);
            await modules.SaveData('gameCurrentTimer', this.gameCurrentTimer);
            await modules.SaveData('gameCurrentNumberOfClears', this.gameCurrentNumberOfClears);
            await modules.SaveData('gameCurrentNumberOfPerfectMatches', this.gameCurrentNumberOfPerfectMatches);
            await modules.SaveData('gameCurrentIsUserGuessWrong', JSON.stringify(this.gameCurrentIsUserGuessWrong));
            await modules.SaveData('gameCurrentNumberOfFails', this.gameCurrentNumberOfFails);
            await modules.SaveData('gameCurrentNumberOfMisses', this.gameCurrentNumberOfMisses);
            await modules.SaveData('gameCurrentHintText', this.gameCurrentHintText);
            await modules.SaveData('gameCurrentTotalScore', this.gameCurrentTotalScore);
            await modules.SaveData('gameCurrentHasAnyPieceEverBeenSelected', JSON.stringify(this.gameCurrentHasAnyPieceEverBeenSelected));
            await modules.SaveData('gameLikenessNudgeHasBeenShown', JSON.stringify(this.gameLikenessNudgeHasBeenShown));
            await modules.SaveData('gameClickMeNudgeHasBeenShown', JSON.stringify(this.gameClickMeNudgeHasBeenShown));
            await modules.SaveData('userHighScoresEasy', JSON.stringify(this.userHighScoresEasy));
            await modules.SaveData('userSettingsUseCats', this.userSettingsUseCats);
            await modules.SaveData('userSettingsUseAltPatterns', this.userSettingsUseAltPatterns);
            await modules.SaveData('userSettingsUseHints', this.userSettingsUseHints);
            ...
            await modules.SaveData('userNumberOfPerfectBasicGames', this.userNumberOfPerfectBasicGames);
            if (this.appSettingsEnableEconomy) {
              await modules.SaveData('userCurrency', JSON.stringify(this.userCurrency));
              await modules.SaveData('userSettingsActionItems', JSON.stringify(this.userSettingsActionItems));
            }
          }
        }
      },
      ...
      async HandleActionItemClick(_item) {
      ...
      async AdjustItemCount(event, _currency, _item, _amount) {
      ...
      async HandleSkipTutorial() {
        ...
        this.appTutorialUserHasSeen = true;
        await modules.SaveData('appTutorialUserHasSeen', true);
      },
      ...
      FillBoardWithPieces(size = this.appSettingsPieceSize) {
        note('FillBoardWithPieces() called');
        if (UseDebug) {
          ...
          let _level = modules.ConstructLevel(numbers, true);
    ...
    beforeUnmount() {
      ...
      window.removeEventListener('pageshow', this.HandleOnPageShowEvent);
      window.removeEventListener('visibilitychange', this.HandleOnVisibilityChange);
      window.removeEventListener('pagehide', this.HandleOnPageHideEvent);
      window.removeEventListener('resize', this.HandleOnResizeEvent);
    },
    ...
    computed: {
      ...
      enabledCurrencies: function () {
        note('enabledCurrencies() called');
      ...
      allPlayerRanksBelowOrEqualToCurrentPlayerRank: function () {
        note('allPlayerRanksBelowOrEqualToCurrentPlayerRank() called');
        return this.allPlayerRanks.filter((r) => r.rank <= this.userRank);
      },
      allPlayerRanksAboveCurrentPlayerRank: function () {
        note('allPlayerRanksAboveCurrentPlayerRank() called');
        return this.allPlayerRanks.filter((r) => r.rank > this.userRank);
      },
    },
  });
  ...
  app.config.compilerOptions.isCustomElement = (tag) =>
      ['app', 'tutorial', 'oobe', 'wallpaper', 'home', 'modes', 'mode', 'piece', 'glyph', 'gild', 'value', 'checkbox', 'toggle', 'version', 'howto', 'icon', 'chrome', 'quit', 'divider', 'stage', 'gameover', 'clears', 'time', 'playarea', 'addtime', 'board', 'me', 'puzzle', 'hint',
      ].includes(tag);
  //#endregion
